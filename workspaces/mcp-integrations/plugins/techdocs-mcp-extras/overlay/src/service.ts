...
export class TechDocsService {
  ...
  // syncTechDocs:: asks the techdocs backend to build/refresh the docs for an
  // entity before we read them — the same endpoint a human visit to the Docs
  // tab triggers. With `techdocs.builder: local` this runs the full
  // prepare→generate→publish pipeline (the backend skips the build when the
  // source is unchanged, and throttles checks per entity); with `builder:
  // external` it is a cheap no-op. The response is an SSE stream that closes
  // when the build finishes, so we drain it with a timeout. Best-effort by
  // design: on any failure or timeout we log and proceed to the content
  // fetch, which then reports the plain not-built-yet error.
  private async syncTechDocs(
    namespace: string,
    kind: string,
    name: string,
    auth?: any,
    ...
    try {
      ...
      this.logger.debug(`Requesting TechDocs sync at URL: ${syncUrl}`);
      const fetch = this.fetchFunction || (await import('node-fetch')).default;
      ...
      if (!response.ok) {
        this.logger.warn(
          `TechDocs sync returned ${response.status} for ${kind}:${namespace}/${name}; proceeding to content fetch`,
        );
        return;
      }
      ...
      await Promise.race([
        ...
        new Promise((_, reject) => {
          timer = setTimeout(
            () => reject(new Error('TechDocs sync timed out after 60s')),
            60_000,
          );
        }),
      ]);
      ...
      this.logger.warn(
        `TechDocs sync failed for ${kind}:${namespace}/${name}; proceeding to content fetch: ${error}`,
      );
    } finally {
      if (timer) {
        clearTimeout(timer);
  ...
  async retrieveTechDocsContent(
    ...
    try {
      ...
      // Build-on-read: trigger the same sync a human visit would, so agents
      // never hit a portal whose docs were simply never built (M18 E2).
      await this.syncTechDocs(namespace, kind, name, auth);
